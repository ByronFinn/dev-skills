...
| **先编排再编码** | `/implement` | 编排 /tdd，类型检查，测试，评审，逐 seam 提交 |
...
| **implement** | `/implement` | Issues / PRD → 已实现代码 + 提交 | 在预约定接缝处编排 /tdd，直接实现非 TDD 项，每项执行类型检查和测试，调度 /review，逐 seam 提交。 |
...
`/implement` 处理逐 seam 编排：适合 TDD 的委托给 `/tdd`，不适合的直接实现，每项执行类型检查和测试，逐 seam 提交，最后调度 `/review`。

### 常用流程

| 场景 | 流程 | 何时使用 |
|---|---|---|
| **新功能** | `/think` → `/grill` → `/story` → `/implement` → `/review` | 从模糊想法到评审通过的完整流程 |
| **直接拆分** | `/story` → `/implement` → `/review` | 已有明确方案，跳过构思环节，自动实现 |
| **缺陷修复** | `/debug` → `/review` (可选) | 报错、崩溃、回归——先定位根因 |
| **技术调研** | `/research` → `/think` (消费 INDEX) | 持久化最佳实践知识供复用 |
| **架构健康检查** | `/improve-architecture` → `/grill` → `/story` → `/implement` | 定期设计债务扫描 |
...
| 🔥 需要挑战方案 | `/grill` |
| 📋 需要将方案拆解为任务 | `/story` |
| ⚙️ 需要将 Issues/PRD 自动实现 | `/implement` |
| ✅ 已确认的待实现需求（单个） | `/tdd` |
| 🐛 报错、崩溃、回归问题 | `/debug` |

...
最常命中的几条(完整规则、全部句式约束、完整替换表见 `write-zh.md` 「句式」与「用词去正式化」两节):

- 禁止破折号（—），用逗号或分号代替
- 四五个短句连发 = 打电报,合成长句或用逗号连
- 不用"首先…其次…最后",用"一方面…另一方面…"
- 词去正式化:`非常/极其`→`很`、`综上所述`→直接收尾、`例如`→`比如`、`购买/使用`→`买/用`、`值得注意的是`→直接说结论(完整表见 write-zh.md)

完整规则、AI 味检测所有模式、标题设计、引号/括号/分号规则见 `write-zh.md`。

长文（多 `##` 节、带表格和图）先做结构再抠句子：跨章节同义清单只留最全一份、表格旁正文别复读、判断/取舍类列表默认转 prose，详见 `write-zh.md` 的「结构级重复与表格复读（长文专项）」，整体流程见 `REFERENCE.md` §Mode Procedures 的 Long-form Article Mode。

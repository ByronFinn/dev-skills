...
5. **Name a consumer skill.** Before a rule enters this table, identify at least one skill whose SKILL.md or REFERENCE.md will cite it by number. A rule with no consumer is dead weight on the always-loaded path. If you can't name a consumer, either fold the rule into an existing cited one, or hold it until a skill needs it.

### Archive candidates

The rules below have **no current consumer** (no skill cites them). They are retained in the table pending a `/grill` decision: cite them in the appropriate skill, fold into a cited sibling, or move to a future `archive` section. Marking them here keeps the debt visible without renumbering (which historically caused citation drift — see audit `docs/audits/2026-07-03-anti-pattern-citation-audit.md`).

High-confidence archive candidates (rare scenarios, or already covered by more specific skill gotchas): #7, #25, #26, #31. The full uncited list is in the audit report.

...
| 25 | Promote one-off report to持久 rule | Commit dated review, scorecard, or diagnostic dump as project guidance, or copy one app's incident details, build numbers, or artifact paths to global rules | Extract only stable invariants. App-specific commands and artifacts stay in project rules, reusable workflows in skills, universal behaviors in global rules, private facts in memory; delete transient reports |
| 26 | Local overlay as source of truth | Rely on rules from ignored or private agent instruction files that future agents, contributors, or packaged installs must obey | Put持久 rules in tracked public docs or shipping skill/rule files. Treat local overlays as optional private context. |
